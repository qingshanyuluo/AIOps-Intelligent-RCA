...
### 2. 训练集的本质性缺失：不仅仅是数据，是“标准化的逻辑”

有人可能会反驳：*“随着 OpenAI o1 等推理模型的发布，或者企业内部私有数据的微调（SFT），模型难道不能学会这些 SOP 吗？”*

我的实战结论是：**很难，甚至在 ROI 上完全不可行。** 根本原因在于**“编程”与“运维”在数据层面存在巨大的物种差异**：

*   **代码（Code）是全球通用的“硬逻辑”：** Python 的语法在 Google 和在我的公司是一样的。Github 上万亿行的代码构成了逻辑严密的训练集，模型很容易收敛。
*   **运维（Ops）是高度定制的“部落知识”：**
    *   我们的 SOP 往往包含大量**公司特有的隐性上下文**（Tribal Knowledge）。例如：“在这个集群，出现 A 报警通常是因为隔壁 B 服务在跑批”。这种逻辑，通用的推理模型（o1）根本无法凭空推导，因为它缺失了最关键的“环境前提”。
*   **数据的“信噪比”陷阱：**
    *   不同于代码的逻辑自洽，企业内部的**故障复盘数据（Post-mortem）** 是极度**非标准化**的。它们往往充斥着试错过程、无效的截图、模糊的自然语言描述，甚至错误的归因。
    *   试图用这些充满杂音（Noise）的数据去微调模型，模型学到的往往不是“精准的排障逻辑”，而是“似是而非的废话”。**要清洗出能让模型收敛的高质量数据集，其成本远高于直接用 Python 固化 SOP。**

因此，不要指望模型能通过“顿悟”或“硬学”来掌握这些复杂的、非标准化的企业级 SOP。**确定性的逻辑，必须由确定性的代码来承载。**
...
### 番外：Gemini 3 Pro 的“基因觉醒” —— 一个有趣的例外

在撰写本文的测试过程中，我发现了一个意味深长的例外：**Google 的 Gemini 3 Pro 在根因定位上的表现，显著超越了 GPT-5 等旗舰模型。**

它甚至能敏锐地捕捉到“下游服务 20ms 的延迟增长，导致了上游线程池耗尽”这种典型的​**分布式连锁反应（Domino Effect）**​。

这是否推翻了我的“数据真空”理论？**不，这恰恰证实了它。**

OpenAI 是模型厂商，但 Google 是​**云厂商**​。

* SRE 文化是 Google 发明的。
* 分布式链路追踪（Dapper）是 Google 发明的。
* **The Tail at Scale（长尾效应）** 是 Google 的研究课题。

Gemini 的“直觉”，极有可能源自 Google 内部几十年来沉淀的、外界无法获取的​**Borg/Colossus 运维数据**​。这再次证明了我的核心观点：**决定 AIOps 上限的，不是 Prompt 技巧，而是模型是否见过“真正的排障轨迹”。**

对于我们（非 Google 内部团队）而言，Gemini 3 Pro 是一个更强的 ​**"Reasoning Node"**​。我们可以让它负责更复杂的“嫌疑人画像”，但为了 100% 的生产安全，**“验证与执行”的权杖，依然必须握在确定性的 SOP 手中（agent能力 gemini 不如 claude）。**
