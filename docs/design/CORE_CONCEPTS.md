...
*   **输入：** 实时 RPC 错误流。
*   **算法：** **Pearson 时空相关性检测**。
*   **判定：** 若 Top N 报错应用的错误曲线 Pearson 系数 $R > 0.85$ 且拓扑离散。
*   **动作：** **直接熔断**。判定为网络专线抖动或云厂商故障，终止单应用分析，输出全局告警。

### Stage 2: 黄金指标采集与补全 (The Dragnet)
**解决痛点：** 仅看报错点无法找到根因（如 Trace 断链）。

既然入口是 RPC，我们利用 **Z-Score 非对称采样算法** 进行“顺藤摸瓜”：
*   **Caller (上游):** 采集报错日志、流量突增情况。
*   **Callee (下游):** 采集 CPU/Mem/GC、K8s Events、DB 慢查。
*   **补全逻辑:** 如果 Trace 中断，但下游某节点 Z-Score 异常（突变），强制将其纳入上下文（Context），通过拓扑关系在内存中重建案发现场。
