...
**解决痛点：**
1.  **LLM 惰性：** Stage 3 输出的“故障全景图”信息量巨大，直接投喂会导致 LLM 忽略关键细节（长文本注意力涣散）。
2.  **逻辑谬误：** LLM 倾向于将最显眼的症状（如 GC 升高）直接归因为根因，缺乏批判性思维。

为此，我们将最终的推理阶段重构为一个 **强制性的两阶段交叉审问 (Two-Phase Cross-Examination)** 流程：

#### **Phase 1: 强制循证与浅层结论提取 (Forced Evidence Review & Shallow Conclusion)**
*   **目的：** 强制 LLM 完整阅读并消化所有证据，防止“偷懒”。
*   **提示词策略 (Prompting Strategy):** 我们不直接问“根因是什么？”，而是分布下发指令：
    1.  “请通读以下所有材料（日志、指标、变更、拓扑）。”
    2.  “以要点形式，总结出所有观测到的、与正常状态不符的异常现象。”
    3.  “基于这些现象，提出一个或多个最直观、最浅显的可能故障猜想。”
*   **输出：** 一份简短的**“案情摘要”**，包含了所有关键异常点和一个初步的、未经审视的“嫌疑人”结论（例如：“初步判断是 A 应用 GC 暂停导致了上游超时”）。

#### **Phase 2: 反事实质询与深度归因 (Counterfactual Inquisition & Deep Attribution)**
*   **目的：** 挑战浅层结论，通过逻辑压力测试，逼近真相。
*   **提示词策略：** 系统将 Phase 1 的浅层结论作为“靶子”，自动生成一系列“反事实假设”来质询 LLM：
    *   **系统质询 1 (挑战因果):** “你初步判断根因是 A 应用的 GC。**反驳开始：** 如果 GC 是根因，为何监控显示其下游 B 应用的自身运算耗时（Self-Time）也在同步增长？这不符合 GC 仅影响自身行为的物理规律。请解释这个矛盾，或者修正你的结论。”
    *   **系统质询 2 (挑战孤证):** “你观察到 CPU 100%。**反驳开始：** 计算 `Impact = ΔLatency × QPS`。如果CPU飙升并未导致QPS或延迟出现显著恶化，那么它更可能是‘结果’而非‘原因’（资源受害者假设）。请重新评估 CPU 在因果链中的位置。”
*   **动作：** LLM 必须在完整的上下文（原始证据 + 案情摘要）中，对这些尖锐的、基于 SRE 第一性原理的质询做出回应。它要么提供更深层次的解释来捍卫并完善其结论，要么推翻初步判断，提出一个新的、更能解释所有证据的根因。
