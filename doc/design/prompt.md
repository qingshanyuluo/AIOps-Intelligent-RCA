...
# gatekeeper:
 """
        守门员节点：拦截 Agent 的最终回复，注入"抗遗忘"指令
        
        当 Agent 想要结束（没有 tool_calls）时，先经过此节点检查，
        强制提醒 Agent 执行"因果倒置验证"，避免将症状误判为根因。
        """
        if self.config.agent.verbose:
            print("\n" + "="*60)
            print("🛡️ Gatekeeper: 执行最终逻辑检查")
            print("="*60)
        
        # 构造"抗遗忘"强制提醒
        reminder_content = """🛑 **SYSTEM INTERVENTION (Final Logic Check)** 🛑

在输出最终结论前，请**强制执行【因果倒置验证】（Step 4）**：

### 验证 4.1: T-1 精确时序判定
- 检查：根因事件的最早时间戳 T_cause 是否 ≤ 故障开始时间 T_error？
- 如果 T_cause > T_error：**结论必定错误**，必须推翻重来！

### 验证 4.2: "资源即受害者"假设（关键！）
**请检查以下情况**：

1. **多组件并发异常检查**：
   - 是否观察到 **多个组件（MySQL + Redis + ES + RPC）同时** 耗时上涨？
   - 如果是，根据《反事实推理原则》，这 **99% 是本服务自身资源（CPU/GC/线程池）问题** 导致的"假性下游慢"
   - **判断标准**：多个组件一起慢 = 自身问题；只有单个组件慢 = 组件问题

2. **资源异常检查**：
   - 如果结论涉及 CPU、内存、GC 或线程池异常，必须自问：
     「有没有可能是外部因素（如下游变慢、上游流量增加）先发生，从而拖垮了本服务的资源？」
   - 如果结论涉及 MySQL/Redis/ES/RPC 等组件变慢，必须自问：
     「服务自身的 GC/CPU 是否同时异常？如果是，组件变慢可能只是被动症状！」

3. **量化核算**：
   - 检查高 QPS 依赖的延迟变化
   - 计算冲击影响：Impact = ΔLatency × QPS
   - 这个冲击是否足以导致当前的资源耗尽？
...
- 如果验证发现**时序矛盾**（T_cause > T_error）：**必须推翻 Step 3 结论，重新定义根因！**
- 如果验证发现**资源是受害者**：**追溯上游流量或下游延迟变化，找到真正触发点！**
- 如果验证发现**多组件同时异常**：**几乎不可能是所有组件都坏了，99%是服务自身GC/CPU问题！**

**只有三项验证全部通过，才能输出最终根因报告。请基于此检查，输出（或修正）你的最终根因分析报告。**"""
        
        # 注入 HumanMessage（利用 LLM 对最后一条指令的高遵循度）
        intervention_message = HumanMessage(content=reminder_content)
